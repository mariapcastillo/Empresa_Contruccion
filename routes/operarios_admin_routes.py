...
from controllers import operarios_admin_controller
...
async def get_all_operarios():
    return await operarios_admin_controller.get_all_operarios()

@router.get("/{operario_id}", status_code=200)
async def get_operario_by_id(operario_id: int):
    return await operarios_admin_controller.get_operario_by_id(operario_id)

@router.post("/", status_code=201)
async def create_operario(data: OperarioCreate):
    return await operarios_admin_controller.create_operario(data)

@router.put("/{operario_id}", status_code=200)
async def update_operario(operario_id: int, data: OperarioUpdate):
    return await operarios_admin_controller.update_operario(operario_id, data)

@router.delete("/{operario_id}", status_code=200)
async def delete_operario(operario_id: int):
    return await operarios_admin_controller.delete_operario(operario_id)